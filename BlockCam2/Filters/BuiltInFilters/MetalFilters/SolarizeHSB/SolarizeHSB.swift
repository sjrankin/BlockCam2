...
class SolarizeHSB: MetalFilterParent, BuiltInFilterProtocol
{
    ...
    required override init()
    {
        ...
        let KernelFunction = DefaultLibrary?.makeFunction(name: "SolarizeHSB")
        do
        {
    ...
    func Initialize(With FormatDescription: CMFormatDescription, BufferCountHint: Int)
    {
        ...
        {
            print("LocalBufferPool is nil in SolarizeHSB.Initialize.")
            return
        }
        ...
        {
            fatalError("Unable to allocate texture cache in SolarizeHSB.")
        }
                   ...
                   _ ColorSpace: CGColorSpace, Options: [FilterOptions: Any]) -> CVPixelBuffer
    {
        ...
        if !Initialized
        {
            Debug.FatalError("SolarizeHSB not initialized at Render(CVPixelBuffer) call.")
        }
        
        let How = Options[.IntCommand] as! Int
        let IfGreater = Options[.IsGreater] as! Bool
        let Threshold = Options[.Threshold] as! Double
        let Parameter = SolarizeHSBParameters(SolarizeHow: simd_uint1(How),
                                            Threshold: simd_float1(Threshold),
                                            SolarizeIfGreater: simd_bool(IfGreater))
        let Parameters = [Parameter]
        ParameterBuffer = MetalDevice!.makeBuffer(length: MemoryLayout<SolarizeHSBParameters>.stride, options: [])
        memcpy(ParameterBuffer.contents(), Parameters, MemoryLayout<SolarizeHSBParameters>.stride)
        ...
        CommandEncoder.label = "SolarizeHSB Kernel"

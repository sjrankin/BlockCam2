...
float4 ToHSB_ForSolarize2(float4 Source)
{
    float r = Source.r;
    float g = Source.g;
    float b = Source.b;
...
kernel void SolarizeHSB(texture2d<float, access::read> InTexture [[texture(0)]],
                           texture2d<float, access::write> OutTexture [[texture(1)]],
                           constant Solarize2Parameters &Solarize [[buffer(0)]],
                           uint2 gid [[thread_position_in_grid]])
{
    ...
    float4 HSB = ToHSB_ForSolarize2(InColor);
    ...
    switch (Solarize.SolarizeHow)
        {
            ...
            if (Solarize.SolarizeIfGreater)
                ...
                if ((InColor.g >= Solarize.Threshold) && (InColor.g <= Solarize.Threshold))
                    {
                    Green = 1.0 - Green;
                    }
                if ((InColor.b >= Solarize.Threshold) && (InColor.b <= Solarize.Threshold))
                    {
                    Blue = 1.0 - Blue;
                    }
                }
            ...
            if (Solarize.SolarizeIfGreater)
                ...
                if ((HSB.r >= Solarize.Threshold) && (HSB.r <= Solarize.Threshold))
                    {
                    Red = 1.0 - Red;
                    Green = 1.0 - Green;
                    Blue = 1.0 - Blue;
